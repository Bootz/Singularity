...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetSingularityString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetSingularityString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetSingularityString(LANG_GM_NOTIFY);
    str += args;
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetSingularityString(LANG_OFFLINE));
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetSingularityString(LANG_ON) : GetSingularityString(LANG_OFF));
        return true;
    }
