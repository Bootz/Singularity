...
{
    const char *format = GetSingularityString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetSingularityString(int32 entry) const
{
    return m_session->GetSingularityString(entry);
}
...
{
    SendSysMessage(GetSingularityString(entry));
}
...
const char *CliHandler::GetSingularityString(int32 entry) const
{
...
{
    return GetSingularityString(LANG_CONSOLE_COMMAND);
}
