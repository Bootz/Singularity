...
{
    char const *format = GetSingularityString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetSingularityString(int32 entry) const
{
    return sObjectMgr->GetSingularityString(entry, GetSessionDbLocaleIndex());
}
